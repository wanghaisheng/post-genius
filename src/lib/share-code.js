const seo = url => `<meta name="twitter:card" content="summary_large_image" />
<meta name="image" content="${url}" />
<meta itemprop="image" content="${url}" />
<meta name="twitter:image" content="${url}" />
<meta property="og:image" content="${url}" />`

const html = url =>
  `<img src="${url}" alt="Generated by https://cards.cards.n0nft.com">`

const markdown = url => `![Generated by https://cards.cards.n0nft.com](${url})`

const javascript = query => {
  const endpoint = query.endpoint
    ? query.endpoint
    : 'https://screenshot.cards.n0nft.com/'
  const payload = new URLSearchParams(query).toString()

  return `const api = '${endpoint}';
const cardUrl = 'https://cards.cards.n0nft.com/?${payload}';
const image = \`\${api}\${encodeURIComponent(cardUrl)}\`;`
}

export const shareCode = {
  seo,
  html,
  markdown,
  javascript
}
